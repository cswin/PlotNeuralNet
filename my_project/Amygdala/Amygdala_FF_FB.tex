...
\draw [connection]  (la1-east)   -- node {\fmidarrow} (la2-west);


%
\path (cr2-east) -- (p2-west) coordinate[pos=0.4] (after2) ;
\draw (d4-near)++(0,0,5.5+\skipshift) coordinate (d4h);
\draw [connection]  (after2)    -- node {\fmidarrow} ++(0,0,13+\skipshift) -- node {\fmidarrow} (score2-west);
%\draw [connection]  (score2-east) -- node{\fmidarrow} (ucr4-west);
\draw [connection]  (ucr4-east) -- node{\fmidarrow} (la1-west);
\draw [connection]  (la2-east) -- node{\fmidarrow} (d4h) -- node{\fmidarrow}(d4-near);
%
